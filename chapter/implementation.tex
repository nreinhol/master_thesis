...
\begin{description}
	\item[\texttt{central\_problem}:]
	\item[\texttt{web3}:]
	\item[\texttt{dealer\_contract}:]
    \item[\texttt{dealer}:]
    \item[\texttt{agent\_pool}:] 
\end{description}

\subsubsection{Main simulation loop}
First of all, the source code of the main simulation loop is outlined in the listing \ref{lst:main_simulation_loop}.
These loop represents the main process of the simulation and is intented to give an initial overview of their
implementation. It takes place after the part \textit{Initial Setup}.
...
the whole simulation process is outlined and the 
implementation of the fundamental parts 
of the simulation process are described in detail
\end{comment}
